...
export default function LoadingScreen() {
  ...
  // Local state for the smooth visual bar
  const [displayProgress, setDisplayProgress] = useState(0);
  const [quoteIndex, setQuoteIndex] = useState(0);

  // --- FIX 1: SMOOTH PROGRESS BAR (Infinite Loop Fixed) ---
  useEffect(() => {
    let animationFrameId: number;

    const animate = () => {
      setDisplayProgress((prev) => {
        // Calculate the distance to the real download progress
        const diff = progress - prev;

        // If very close to the target, snap to it (prevents infinite micro-decimals)
        if (Math.abs(diff) < 0.5) {
          return progress;
        }

        // Ease towards the target (10% per frame)
        return prev + diff * 0.1; 
      });

      // Keep animating as long as we aren't at 100% or three.js is still active
  ...
  }, [progress, active]); // <--- REMOVED 'displayProgress' to stop the crash

  // --- FIX 2: STORE SYNC ---
  useEffect(() => {
    const roundedValue = Math.round(displayProgress);
    
    // Sync with global store
    setStoreProgress(roundedValue);

    // If visual bar hits 100%, ensure store knows we are done
    // Note: We don't set isLoaded here manually; the store handles that when progress hits 100
  }, [displayProgress, setStoreProgress]); 
